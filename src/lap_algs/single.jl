...
function single_lap(image_1::Image,
                    ...
                    timer::TimerOutput=TimerOutput("single_lap"),
                    display::Bool=false)
    ...
    @timeit_debug timer "prepare A and b" begin
        ...
        for k in 1:filter_num-1
            for l in k:filter_num-1
                @timeit_debug timer "window sum 1" @views window_sum3!(A[k, l, :], dif[:, k+1] .* dif[:, l+1], image_size, w)
                A[l, k, :] = A[k, l, :]
            end
            @timeit_debug timer "window sum 2" @views window_sum3!(b[k, :], dif[:, k+1] .* dif[:, 1] .* (-1), image_size, w)
        end
    end

    # Perform Gauss elimination on all pixels in parallel:
    # coeffs will be of shape: pixel_count, filter_num-1
    @timeit_debug timer "multli mat div" begin
    ...
    if (display && getfield(LAP_julia.lap, :timeit_debug_enabled)())
        print_timer(timer)
        println()
    end
    return u_est
...
function single_lap_at_points(image_1::Image, image_2::Image, filter_half_size::Integer, window, filter_num, points)
